...
const ProposalsPage: React.FC = () => {
  ...
  // NIEUW: We slaan ook de index op, zodat we kunnen verwijderen vanuit de popup
  const [viewingState, setViewingState] = useState<ViewingState | null>(null);

  // --- FIX 1: CITO OOK VIA PROXY ---
  ...
  const handleRemoveSource = (proposalIndex: number, sourceId: string) => {
      ...
      if (prop) {
          ...
          // Als we deze bron aan het bekijken waren in de popup, sluit de popup
  ...
  if (loading || generatingLesson) return <div className="min-h-screen flex items-center justify-center bg-gray-100 font-bold text-gray-600">🤖 De AI is hard aan het werk... ({generatingLesson ? 'Les schrijven' : 'Concepten bedenken'})</div>;
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8 font-sans">
      <div className="max-w-7xl mx-auto">
              ...
              <div className="p-5 flex-1 flex flex-col min-h-0">
                <div className="overflow-y-auto pr-2 space-y-6 flex-1">
                    ...
                    <div>
                        ...
                        <p className="text-gray-600 text-sm leading-relaxed">{prop.rationale}</p>
                    </div>

                    {/* Feedback */}
                    <div className="bg-gray-50 p-3 rounded border border-gray-200">
                        <label className="text-[10px] font-bold text-gray-500 uppercase block mb-1">Stuur de AI bij</label>
                    ...
                    <div>
                        <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 flex justify-between">
                            ...
                            <span className="bg-gray-100 px-2 rounded text-gray-600">{safeIds.length}</span>
                        </h4>
                        ...
                        <div className="space-y-2">
                            {safeIds.map((sourceId, listIdx) => {
                                ...
                                return (
                                    <div 
                                        key={sourceId} 
                                        // HIER: We slaan ook de proposalIndex op!
                                        onClick={() => setViewingState({ source, proposalIndex: idx })}
                                        className={`
                                            relative flex gap-3 p-2 rounded border cursor-zoom-in transition-colors group
                                            ${isTop ? 'border-amber-300 bg-amber-50' : 'border-gray-100 bg-white hover:border-indigo-200'}
                                        `}
                                    >
                                        {isTop && (
                                            <div className="absolute -top-2 -left-1 text-xs bg-white rounded-full shadow-sm border border-amber-200 px-1">⭐</div>
                                        )}
                                        ...
                                        <div className="w-12 h-12 bg-gray-200 shrink-0 rounded overflow-hidden">
                                            {imgUrl ? (
                                                ...
                                                <div className="w-full h-full flex items-center justify-center text-[8px] text-gray-400">Geen beeld</div>
                                            )}
                                        </div>

                                        <div className="flex-1 min-w-0">
                                            <div className="text-xs font-bold text-gray-900 truncate">{source.title}</div>
                                            <div className="text-[10px] text-gray-500 truncate">{source.provider} • {source.type}</div>
                                        </div>

                                        {/* FIX 2: Grotere hit-area voor het kruisje */}
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); handleRemoveSource(idx, source.id); }}
                ...
                <div className="pt-4 mt-2 border-t border-gray-100">
                    <button onClick={() => handleChoose(prop)} className="w-full py-3 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700 transition-colors shadow-md flex justify-center items-center gap-2">
                        Start Les Maken →
                    </button>
                </div>
        ...
        {/* POPUP MODAL (MET VERWIJDER KNOP) */}
        {viewingState && (
                ...
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-full overflow-y-auto flex flex-col" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="p-6 border-b flex justify-between items-start">
                        <div>
                            <span className="text-xs font-bold text-indigo-600 uppercase tracking-wide mb-1 block">
                                {viewingState.source.provider} • {viewingState.source.type}
                            </span>
                            <h2 className="text-xl font-bold text-gray-900">{viewingState.source.title}</h2>
                        </div>
                        <button onClick={() => setViewingState(null)} className="text-gray-400 hover:text-black text-2xl">&times;</button>
                    </div>
                    ...
                    <div className="p-6 overflow-y-auto">
                        ...
                        <div className="prose prose-sm max-w-none text-gray-700">
                            <p className="whitespace-pre-wrap">
                                {viewingState.source.fullText || viewingState.source.content || viewingState.source.description || "Geen tekst beschikbaar."}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="p-4 border-t bg-gray-50 flex justify-between items-center">
                        {viewingState.source.url ? (
                            <a href={viewingState.source.url} target="_blank" rel="noopener noreferrer" className="text-indigo-600 font-bold hover:underline text-sm">
                                Bekijk origineel ↗
                            </a>
                        ) : <div></div>}
                        ...
                        <div className="flex gap-3">
                            ...
                            <button 
                                onClick={() => handleRemoveSource(viewingState.proposalIndex, viewingState.source.id)}
