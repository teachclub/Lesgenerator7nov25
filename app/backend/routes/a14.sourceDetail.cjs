...
module.exports = function router() {
    ...
    for (const el of elems) {
      ...
      if (tag === "p") {
        ...
        for (const line of lines) {
          ...
          if (/^Fragment\s+\d+\b/i.test(line)) push(`### ${line}`);
          else push(line);
        }
        continue;
      }
  ...
  // POST /api/source-detail  { url }
  r.post("/source-detail", async (req, res) => {
    try {
      ...
      if (!isKleioUrl(url)) {
        return res.status(400).json({
          ok: false,
          error: "Alleen vgnkleio.nl/bronnen URLs toegestaan",
        });
      }
      ...
      return res.status(500).json({ ok: false, error: e?.message || "source-detail crash" });
    }
  });
