...
export const SelectionPanel = ({ onSelectSource, selectedId }: Props) => {
  ...
  const getImageUrl = (source: Source) => {
    ...
    const needsProxy =
      source.provider === "Cito" || source.provider === "Kleio" || url.includes("kleio");
            ...
            <div className="p-3 flex-1 min-w-0 flex flex-col justify-between">
              <div className="min-w-0">
                <div className="flex justify-between items-start pr-4 min-w-0">
                  <h3
                    className="font-bold text-gray-900 text-sm line-clamp-2 leading-snug break-words min-w-0"
                    title={source.title}
                  >
                    {source.title}
                  </h3>
                ...
                <p className="text-xs text-gray-500 line-clamp-3 mt-1 leading-relaxed break-words min-w-0">
              ...
              <div className="flex justify-between items-center mt-2 pt-2 border-t border-gray-50">
                {!hasImage ? (
                  <span className="text-[10px] font-bold text-gray-400 uppercase truncate max-w-[60%]">
                    {source.provider}
                  </span>
                ) : (
                  <span />
                ...
                <div className="flex gap-2 items-center">
                  <span
                    className={`text-[10px] px-1.5 rounded font-bold ${
                      source.type === "IMAGE" ? "text-purple-700 bg-purple-50" : "text-blue-700 bg-blue-50"
                    }`}
                  >
