...
import { useSelectionStore } from '../state/selection.store'; // We hebben types nodig
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';

const LessonPage = () => {
  const { lessonPlan, setLessonPlan } = useLessonStoreV2();
  const [sources, setSources] = useState<any[]>([]); // Lokale state voor sources
  const [generationStep, setGenerationStep] = useState<number>(0);
  const [error, setError] = useState('');

  useEffect(() => {
     // 1. Probeer Router State
     let startData = window.history.state?.usr;
     
     // 2. Probeer LocalStorage (voor nieuw tabblad)
     if (!startData || !startData.concept) {
         const stored = localStorage.getItem('lessonContext');
         if (stored) {
     ...
     if (startData && startData.concept) {
         setSources(startData.sources || []);
         // Alleen starten als er nog geen plan is
         if (!lessonPlan || lessonPlan.title !== startData.concept.title) {
             startGeneration(startData.concept, startData.sources || []);
         }
     }
  }, []);
  ...
  const startGeneration = async (concept: any, usedSources: any[]) => {
      ...
      try {
          ...
          if(!res1.ok) throw new Error("Stap 1 mislukt");
          ...
          setLessonPlan({
              ...part1,
              phases: part2.phases,
              teacherGuide: { ...part1.teacherGuide, reflectionAnswers: part4.reflection?.answers, filledTables: part4.filledTables },
              studentWorksheet: { ...part3.studentWorksheet, sourceAnalyses: part4.sourceAnalyses, reflectionQuestions: part4.reflection?.questions }
          });
          setGenerationStep(0);
  ...
  if (!lessonPlan) return <div className="p-10 text-center">Laden... (Of geen data gevonden)</div>;
    ...
    <div className="min-h-screen bg-white pb-20 print:p-0 font-sans text-sm text-gray-900">
      ...
      <div className="max-w-[297mm] mx-auto p-8 bg-white print:max-w-none print:p-0">
           ...
           <div className="columns-2 gap-8">
             {displaySources.map((s, i) => (
               <div key={s.id} className="avoid-break mb-6 border-2 border-black p-3 bg-white">
                  ...
                  {/* Tekst + Context (Belangrijk!) */}
                  <div className="text-xs font-serif text-justify">
                      <ReactMarkdown>{s.content || s.description || "Geen tekst."}</ReactMarkdown>
                  </div>
                  <div className="text-[10px] italic text-right mt-2 text-gray-500 border-t pt-1">
                      Oorsprong: {s.origin || s.provider}
                  </div>
               </div>
             ))}
           </div>
        ...
        <section className="mb-8 page-break-after">
           ...
           <div className="grid grid-cols-4 gap-2 mb-8">{sheet?.grabBag?.map((bag:any, i:number) => (<div key={i} className="border-2 border-black p-2 text-xs"><strong className="block border-b border-black mb-1 bg-black text-white px-1">{bag.label}</strong>{bag.items?.join(', ')}</div>))}</div>
           <div className="mb-8 avoid-break"><h3 className="font-bold text-lg mb-2">Opdracht 1: Samenwerkingstabel</h3><ReactMarkdown remarkPlugins={[remarkGfm]} components={MarkdownComponents}>{sheet?.emptyTables?.collaboration || ''}</ReactMarkdown></div>
           <div className="mb-8 avoid-break"><h3 className="font-bold text-lg mb-2">Opdracht 2: Positioneringskwadrant</h3><ReactMarkdown remarkPlugins={[remarkGfm]} components={MarkdownComponents}>{sheet?.emptyTables?.quadrant || ''}</ReactMarkdown></div>
           {sheet?.sourceAnalyses?.map((sa:any, i:number) => (<div key={i} className="mb-6 avoid-break"><div className="bg-black text-white px-3 py-1 font-bold inline-block mb-1 text-sm">Vragen bij Bron {i+1}</div><div className="border-2 border-black p-3">{sa.questions?.map((q:string, j:number) => (<div key={j} className="mb-4 last:mb-0"><p className="font-bold text-xs mb-4">{j+1}. {q}</p><div className="border-b border-gray-300 h-6"></div></div>))}</div></div>))}
        ...
        <section>
          ...
          {guide?.filledTables && (<div className="mb-8"><h3 className="font-bold text-red-700 mb-2">Ingevulde Tabellen</h3><div className="text-red-900"><ReactMarkdown remarkPlugins={[remarkGfm]} components={MarkdownComponents}>{guide.filledTables.collaboration || ''}</ReactMarkdown></div><div className="mt-4 text-red-900"><ReactMarkdown remarkPlugins={[remarkGfm]} components={MarkdownComponents}>{guide.filledTables.quadrant || ''}</ReactMarkdown></div></div>)}
